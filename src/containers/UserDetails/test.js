...
class App extends Component {
  ...
  addBook() {
    axios.post('http://localhost:3004/User', this.state.newBookData).then((response) => {
  ...
  updateBook() {
    ...
    axios.put('http://localhost:3004/User/' + this.state.editBookData.id, {
      title, rating
    }).then((response) => {
      this._refreshBooks();
  ...
  deleteBook(id) {
    axios.delete('http://localhost:3004/User/' + id).then((response) => {
      this._refreshBooks();
    });
  }
  _refreshBooks() {
